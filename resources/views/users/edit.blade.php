...
@section('content')
    <div class="col-md-8">
        ...
        <form enctype="multipart/form-data" class="bg-white shadow-sm p-3"
            ...
            <br>
            <input type="num" name="phone" class="form-control {{ $errors->first('phone') ? 'is-invalid' : '' }}"
            ...
            Current avatar: <br>
            @if ($user->avatar)
                <img scr="{{ asset('storage/' . $user->avatar) }}" width="120px" />
            @else
                No avatar
            @endif
            <br>
            ...
            <small class="text-muted">
                Kosongkan jika tidak inin mengubah avatar
            </small>

            <hr class="my-3">
            <label for="email">Email</label>
            <input value="{{ $user->email }}" disabled
                class="form-control {{ $errors->first('email') ? 'is-invalid' : '' }}" placeholder="user@gmail.com"
